...
Bot classification:
- Review bots provide actionable code feedback. Their comments are categorized
  by content into high/medium/low with a ``review_bot: true`` flag — they are
...
def detect_priority_marker(body: str) -> str | None:
    """Detect high/medium/low priority markers in comment body.
    ...
    """
    # Check for priority markers at start of comment (with optional whitespace)
    priority_patterns = [
...
def categorize_comment(comment: dict[str, Any], body: str) -> str:
    ...
    Uses high/medium/low priority markers when present.
    """
    ...
    priority_level = detect_priority_marker(body)
    if priority_level:
...
def main():
    ...
    # Categorized feedback using high/medium/low priorities
    feedback = {
    ...
    # Add actionable summary based on high/medium/low priorities
    if feedback['high']:
